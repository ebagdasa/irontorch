...
mask = np.zeros([32, 32, 3], dtype=np.float32)

reshaped_mask = mask.reshape(-1)
s = int(reshaped_mask.shape[0] * 0.1)
start_pos = 0 #random.randint(0, reshaped_mask.shape[0] - s - 1)
reshaped_mask[start_pos:start_pos + s] = 1
pattern = np.ones(shape=(mask.shape[0], mask.shape[1], 3))
...
def make_dataloaders(train_dataset=None, val_dataset=None, batch_size=None, num_workers=None):
    ...
    for name in ['train', 'test', 'poison']:
        label_pipeline: List[Operation] = [IntDecoder()]
        image_pipeline: List[Operation] = [SimpleRGBImageDecoder()]
        train_indices = np.random.choice(list(range(60000)), 100, replace=False)
        val_indices = np.arange(20000)
        ...
        if name == 'train':
            image_pipeline.extend([
                ...
                Poison(mask=mask, indices=train_indices, pattern=pattern),
            ])
            label_pipeline.extend(
                [ReplaceLabel(indices=train_indices, new_label=5)])
        if name == 'poison':
            image_pipeline.extend([
                Poison(mask=mask, indices=val_indices, pattern=pattern)
            ])
            label_pipeline.append(ReplaceLabel(indices=val_indices, new_label=5))
        ...
        label_pipeline.extend([ToTensor(), ToDevice('cuda:0'), Squeeze()])
        loaders[name] = Loader(paths[name], batch_size=batch_size, num_workers=num_workers,
                               order=ordering, drop_last=(name == 'train'), #recompile=True,
...
def train(model, loaders, lr=None, epochs=None, label_smoothing=None,
    ...
    for epoch in range(epochs):
        for ims, labs in tqdm(loaders['train'], desc=f'Training. Epoch {epoch}'):
...
def evaluate(model, loaders, lr_tta=False):
    ...
    with ch.no_grad():
        for name in ['train', 'test', 'poison']:
            ...
            for ims, labs in tqdm(loaders[name], desc='Evaluating {}'.format(name)):
                with autocast():
                    out = model(ims)
                    if lr_tta:
