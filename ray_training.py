...
if __name__ == '__main__':
    ...
    if args.task == 'mnist':
        ...
        epochs = 10
        proportion_to_test = np.unique(np.logspace(0, 6, num=20, base=2, dtype=np.int32)).tolist()
    ...
    if (args.poisoning_proportion is None) and (args.load_stage3 is None):
        ...
        for space in [0.01, 0.02, 0.05, 0.1, 0.2, 0.5]:
            # stage 2
    ...
    if not args.load_stage3:
        ...
        search_space = {
            ...
            "group": group_name,
            "decay": tune.qloguniform(1e-7, 1e-3, 1e-7, base=10),
            "epochs": tune.randint(2, epochs + 2),
            'random_seed': random_seed,
            "backdoor_cover_percentage": args.backdoor_cover_percentage,
            "batch_size": batch_size,
    ...
    if args.stage4_multi_backdoor:
        synthesizers =['NarcissusClean', 'Primitive', 'SinglePixel',  'Complex', 'Dynamic']
    else:
        synthesizers = [args.synthesizer]
